...
func generate():
	...
	if room_positions.size() > 0:
		spawn_position = room_positions[0] * grid_map.cell_size
		spawn_position.y = 3
	...
	await create_hallways(hallway_graph)
	_generated = true
	emit_signal("dungeon_generated")
	spawn_point_ready.emit(spawn_position)
...
func create_hallways(hallway_graph:AStar2D) -> void:
	...
	astar.region = Rect2i(0, 0, border_size, border_size) 
	astar.update()
